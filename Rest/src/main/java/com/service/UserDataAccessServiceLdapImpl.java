...
public class UserDataAccessServiceLdapImpl implements UserDataAccess {
	...
	public UserInformation queryByUsername(String username) {
		LOGGER.info("queryByUsername in LDAP...");
		return getUserInformation(generateQueryByUsername(username));
	}
	
	@Override
	public String getUserNameByUserId(String userId) {
		LOGGER.info("getUserNameByUserId in LDAP...");
	...
	private UserInformation getUserInformation(LdapQuery query) {
		...
		if(users != null && users.size() == 1)	{
			...
			//throw new RuntimeException("Found "+users.size()+" entries in LDAP.");
